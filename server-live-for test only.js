                            ...
                            try {
                                ...
                                if (Array.isArray(parsedData)) {
                                    ...
                                    db.run(
                                        ...
                                        [manualId, req.user.id, act.name || 'Manual Workout', act.sport_type || 'Workout', act.distance_km || 0, act.moving_time_min || 0, startDate, sparkScore],
                                        (err) => {
...
6. PREDICTIVE LOGISTICS: If the WEATHER ALERT is present and the athlete has an outdoor workout (e.g. Bike or Run) scheduled for today, you MUST proactively ask if they want to convert today's outdoor session into an indoor Zwift/treadmill session due to the miserable weather. For example: "Looks miserable out there today. Do you want me to convert today's ride into an indoor Zwift session?"`;
...
app.post("/api/admin/add-tokens", authenticateToken, (req, res) => {
    const isRutger = req.user.username && req.user.username.toLowerCase().includes("rutger");
    const isFelix = req.user.username && req.user.username.toLowerCase().includes("felixson");
    if (!isRutger && !isFelix && req.user.id !== 1) {
        return res.status(403).json({ error: "Unauthorized" });
...
app.delete("/api/admin/delete-user/:targetUsername", authenticateToken, (req, res) => {
    const isRutger = req.user.username && req.user.username.toLowerCase().includes("rutger");
    const isFelix = req.user.username && req.user.username.toLowerCase().includes("felixson");
    if (!isRutger && !isFelix && req.user.id !== 1) {
        return res.status(403).json({ error: "Unauthorized" });
...
async function tagStravaActivity(userId, activity, token) {
    ...
    const sparkScore = activity.suffer_score || Math.round((activity.moving_time / 3600) * 50);
...
// Use http.createServer so we can attach WS
...
server.on('upgrade', (request, socket, head) => {
    ...
    if (pathname === '/ws/admin/live') {
        ...
        jwt.verify(token, process.env.JWT_SECRET || 'spark-super-secret-key-2026', (err, user) => {
            if (err) {
                socket.write('HTTP/1.1 401 Unauthorized\r\n\r\n');
                socket.destroy();
                return;
            }
            const isRutger = user.username && user.username.toLowerCase().includes('rutger');
            const isFelix = user.username && user.username.toLowerCase().includes('felixson');
            if (!isRutger && !isFelix && user.id !== 1) {
...
wss.on('connection', (ws, request, user) => {
    console.log(`Live API connection started for user ${user.username}`);
    
    db.get(`SELECT athlete_context, training_phase, coach_tone, long_term_memory FROM users WHERE id = ?`, [user.id], (err, dbUser) => {
        const tone = dbUser?.coach_tone || 'Professional';
        const phase = dbUser?.training_phase || 'General';
        const context = dbUser?.athlete_context || 'No specific background provided.';
        const longTermMemory = dbUser?.long_term_memory || 'None recorded.';
        
        db.all(`SELECT metric, value FROM athlete_metrics WHERE user_id = ?`, [user.id], (err, metricsRows) => {
            const metricsText = (metricsRows && metricsRows.length > 0)
                ? metricsRows.map(m => `${m.metric}: ${m.value}`).join(', ')
                : 'None explicitly recorded yet.';

            db.all(`SELECT name, sport_type, distance_km, moving_time_min, spark_score, start_date FROM activities WHERE user_id = ? ORDER BY start_date DESC LIMIT 3`, [user.id], (err, recentActivities) => {
                const recentActivitiesText = (recentActivities && recentActivities.length > 0)
                    ? recentActivities.map(a => `- ${getAMSDateString(a.start_date)}: ${a.name} (${a.sport_type}) | ${parseFloat(a.distance_km).toFixed(1)}km | ${Math.round(a.moving_time_min)}min | ${Math.round(a.spark_score || 0)} Spark`).join('\n')
                    : 'No recent activities recorded.';
                ...
                const futureStr = getAMSDateString(futureDate);
                
                db.all(`SELECT date, sport, description FROM micro_plan WHERE user_id = ? AND date >= ? AND date <= ? ORDER BY date ASC`, [user.id, todayStr, futureStr], (err, plannedWorkouts) => {
                    const planMap = {};
                    (plannedWorkouts || []).forEach(w => {
                        planMap[w.date] = planMap[w.date] ? planMap[w.date] + ` and ${w.sport}: ${w.description}` : `${w.sport}: ${w.description}`;
                    });
                    
                    const next7Days = Array.from({ length: 7 }, (_, i) => {
                        const d = new Date();
                        d.setDate(d.getDate() + i);
                        const dStr = getAMSDateString(d);
                        const workout = planMap[dStr] || 'Rest / No workout scheduled';
                        return `${getAMSWeekday(d)} ${dStr}: ${workout}`;
                    }).join('\n');
                    ...
                    ws.on('message', (data, isBinary) => {
                        if (!geminiWs) {
                            try {
                                ...
                                // If it's an init message, grab the history and initialize
                                // If it's NOT an init message (cached client), initialize anyway without history
                                if (msg.type === "init") {
                                    historyText = msg.history || "No recent history.";
                                }
...
CRITICAL RULES:
1. You MUST speak directly to the athlete using natural, conversational language. DO NOT use asterisks for internal thoughts or reasoning (e.g. avoid **Planning a run**).
2. If the athlete asks about their schedule, read them the schedule provided in TIME CONTEXT.
3. When the athlete asks to schedule a new workout or modify an existing one, you MUST IMMEDIATELY use the schedule_workout function call API to update the database. DO NOT output "Ztool_code" or python code.
4. You MUST ALWAYS trigger the schedule_workout tool when discussing schedule changes. Never just say you updated it without actually calling the tool.
5. Keep your spoken response purely conversational. Do NOT speak any JSON, technical details, or tool names.`;
                                                ...
                                                functionDeclarations: [
                                                    {
                                                        ...
                                                        description: "Schedule a new workout or modify an existing one on a specific date.",
                                                        parameters: {
                                                            type: "OBJECT",
                                                            properties: {
                                                                date: { type: "STRING", description: "YYYY-MM-DD format" },
                                                                sport: { type: "STRING", description: "REQUIRED. MUST be exactly one of: 'Run', 'Bike', 'Swim', 'Strength', 'Rest'." },
                                                                description: { type: "STRING", description: "Short description of the workout" },
                                                                steps_json: { 
                                                                    type: "STRING",
                                                                    description: "A stringified JSON array of workout steps. Each step MUST have 'type' (warmup, main set, cool-down, recovery, interval), 'condition_type' (time, distance, open), and 'condition_value'."
                                                                }
                                                            },
                                                            required: ["date", "sport", "description", "steps_json"]
                                                        }
                                                    }
                                                ]
                                            }]
                                ...
                                geminiWs.on('message', (geminiData, geminiIsBinary) => {
                                    try {
                                        const msgStr = geminiData.toString();
                                        require('fs').appendFileSync('gemini_debug.txt', msgStr + '\n');
                                        const geminiMsg = JSON.parse(msgStr);
                                        
                                        if (geminiMsg.error) {
                                            console.error("Gemini Live API Error:", geminiMsg.error);
                                        }
                                        if (geminiMsg.toolCall && geminiMsg.toolCall.functionCalls) {
                                            for (const functionCall of geminiMsg.toolCall.functionCalls) {
                                                handleFunctionCall(functionCall, geminiWs, user.id);
                                            }
                                        }
                                    } catch (e) {}
                                    
                                    if (ws.readyState === WebSocket.OPEN) {
                                        ws.send(geminiData, { binary: geminiIsBinary });
                                    }
                                });
                                
                                geminiWs.on('close', (code, reason) => {
                                    console.log(`Gemini connection closed. Code: ${code}, Reason: ${reason}`);
                                    require('fs').appendFileSync('gemini_debug.txt', `CLOSED ${code} ${reason}\n`);
                                ...
                                console.error("Error before geminiWs initialized:", e.message, "Data:", data.toString().substring(0, 50));
                            }
                        } else {
...
async function handleFunctionCall(functionCall, geminiWs, userId) {
    console.log("Gemini triggered function:", functionCall.name, functionCall.args);
    let result = { success: false };
    
    try {
        if (functionCall.name === 'schedule_workout') {
            const { date, sport, description, steps_json } = functionCall.args;
            
            // steps_json is now an ARRAY of objects from Gemini.
            // We need to stringify it before saving to SQLite.
            const stepsString = Array.isArray(steps_json) ? JSON.stringify(steps_json) : (steps_json || '[]');
            
            await new Promise((resolve, reject) => {
                // Delete existing workout for the same date and sport to prevent duplicates
                db.run(`DELETE FROM micro_plan WHERE user_id = ? AND date = ? AND sport = ?`, [userId, date, sport], function(err) {
                    if (err) return reject(err);
                    
                    db.run(`INSERT INTO micro_plan (user_id, date, sport, description, target_spark, details, steps_json) VALUES (?, ?, ?, ?, ?, ?, ?)`,
                    [userId, date, sport, description, null, null, stepsString], function(err) {
                        if (err) reject(err);
                        else resolve();
                    });
                });
            });
            result = { success: true, message: `Workout scheduled on ${date}` };
            sendSSEEvent(userId, 'sync_complete', { message: 'Workout scheduled' }); // trigger frontend refresh
        }
    } catch (e) {
        result = { success: false, error: e.toString() };
    }
    
    // In Multimodal Live API, tool responses must be sent via 'toolResponse', not 'clientContent'.
    const responseMsg = {
        toolResponse: {
            functionResponses: [{
                id: functionCall.id,
    ...
    if (geminiWs.readyState === WebSocket.OPEN) {
        ...
        require('fs').appendFileSync('gemini_debug.txt', 'SENT TOOL RESPONSE: ' + JSON.stringify(responseMsg) + '\n');
    }
}
