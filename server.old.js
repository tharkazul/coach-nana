...
try {
  garminExercises = JSON.parse(
    fs.readFileSync("./garmin_exercises.json", "utf8"),
  );
  console.log(
...
setInterval(() => {
  ...
  db.all(
    ...
    `,
    [twentyFourHoursAgo],
    async (err, inactiveUsers) => {
      if (err || !inactiveUsers) return;

      for (const user of inactiveUsers) {
        console.log(
          `🤖 User ${user.id} inactive for 24h. Generating proactive message...`,
        );
...
setInterval(() => {
  ...
  if (
    (now.getHours() === 15 || now.getHours() === 20) &&
    now.getMinutes() === 0
  ) {
